...
watch(data, (newData) => {
  ...
  const { parentId, content, id } = parsedData;
  if (parentId !== null) {
...
function addReply(parentId: number, replyContent: string): void {
  if (lastProcessedReplyId === parentId) return;
  const parentComment = findComment(comments.value, parentId);
  if (!parentComment) return; //it helps to get early return
  const newReply: Comment = {
    id: Date.now(), //It makes a unique number
...
function findComment(
  ...
  return commentsList.reduce<Comment | null>((foundComment, comment) => {
    if (foundComment) return foundComment;
    if (comment.id === commentId) {
      return comment;
    }
    return findComment(comment.children, commentId);
  }, null);
}
